...
class DirectoryTree:
    def __init__(self, root_dir, dir_only=False, output_file=sys.stdout, all=False):
        self._output_file = output_file
        self._generator = _TreeGenerator(root_dir, dir_only, all)
...
class _TreeGenerator:
    def __init__(self, root_dir, dir_only=False, all=False):
    ...
    def _prepare_entries(self, directory):
        ...
        if self._dir_only and self._all:
            entries = [entry for entry in entries if entry.is_dir()]
        if self._dir_only:
            entries = [entry for entry in entries if entry.is_dir() and not entry.name.startswith(".")]
            return entries
        if not self._all:
